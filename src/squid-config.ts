...
/**
 * DLP (Data Loss Prevention) regex patterns for detecting sensitive data
 *
 * These patterns are designed to catch common credential formats in HTTP requests.
 * They are used with Squid's url_regex ACL to detect sensitive data in URLs,
 * query strings, and path segments.
 *
 * Note: For full request body inspection (POST data), ICAP would be needed.
 * This implementation focuses on URL-based detection which catches many
 * common exfiltration attempts via query parameters and API paths.
 */
export const DLP_PATTERNS = {
  // GitHub tokens (all formats)
  githubToken: 'gh[pors]_[A-Za-z0-9]{36}',
  githubPatToken: 'github_pat_[A-Za-z0-9]{22}_[A-Za-z0-9]{59}',
  ...
  // Generic patterns - more targeted to reduce false positives
  // Matches: api_key=xxx, apiKey=xxx, api-key=xxx in query strings
  genericApiKey: '(?:api[_-]?key|apikey)=[A-Za-z0-9_-]{16,}',
  genericToken: '(?:token|access_token|auth_token)=[A-Za-z0-9_-]{16,}',
  genericSecret: '(?:secret|client_secret)=[A-Za-z0-9_-]{16,}',
};

/**
 * Generates DLP (Data Loss Prevention) configuration section for Squid
 *
 * This creates ACL rules to detect and block requests containing sensitive
 * data patterns in URLs. The patterns are checked using Squid's url_regex ACL.
 *
 * @returns Squid configuration string for DLP rules
 */
function generateDlpSection(): string {
  const patterns = [
    DLP_PATTERNS.githubToken,
    DLP_PATTERNS.githubPatToken,
    DLP_PATTERNS.openaiKey,
    DLP_PATTERNS.awsAccessKey,
    DLP_PATTERNS.genericApiKey,
    DLP_PATTERNS.genericToken,
    DLP_PATTERNS.genericSecret,
  ];

  // Generate ACL entries for each pattern
  const aclLines = patterns.map((pattern, i) =>
    `acl dlp_sensitive_${i} url_regex -i ${pattern}`
  ).join('\n');

  // Create deny rules for each pattern
  const denyLines = patterns.map((_, i) =>
    `http_access deny dlp_sensitive_${i}`
  ).join('\n');

  return `
# DLP (Data Loss Prevention) Configuration
# Detects and blocks requests containing sensitive data patterns
# Patterns are checked in URLs, query strings, and path segments
# Note: For full request body inspection, ICAP would be needed

# DLP ACL definitions for sensitive data patterns
${aclLines}

# Deny requests matching DLP patterns (applied before domain rules)
...
http_access deny CONNECT !Safe_ports`;
  ...
  const dlpSection = enableDlp ? generateDlpSection() : '';
...
${enableDlp ? '\n# DLP mode enabled - requests will be scanned for sensitive data patterns' : ''}
