...
export function generateSquidConfig(config: SquidConfig): string {
  ...
  if (filteredPlainDomains.length > 0 && patterns.length > 0) {
    ...
    // No domains - allow all with warning (modified from main's deny all)
    aclSection = '# WARNING: No domain restrictions configured - all outbound traffic is allowed';
    denyRule = '';  // No deny rule means traffic is allowed
  }

  // Format the deny rule section
  const denyRuleSection = denyRule
