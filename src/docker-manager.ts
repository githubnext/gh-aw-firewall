 ...
 */
async function generateRandomSubnet(): Promise<{ subnet: string; squidIp: string; agentIp: string }> {
...
export function generateDockerCompose(
  ...
  const agentService: any = {
    ...
    volumes: [
      // Mount host filesystem for agent access
      '/:/host:rw',
      '/tmp:/tmp:rw',
  ...
  try {
    if (fs.existsSync(workDir)) {
      ...
      if (fs.existsSync(agentLogsDir) && fs.readdirSync(agentLogsDir).length > 0) {
        const preservedLogsDir = path.join(os.tmpdir(), `agent-logs-${timestamp}`);
        try {
