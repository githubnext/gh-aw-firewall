...
const createTempDirs = () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'firewall-wrapper-int-'));
...
describe('firewall-wrapper CLI integration', () => {
  let rootDir: string;
  let binDir: string;
  let workDir: string;
  let logFile: string;
  ...
  it('runs the CLI end-to-end and cleans up work directory when containers are not kept', async () => {
    ...
    expect(commands).toEqual([
      ...
      'inspect firewall-wrapper-copilot --format={{.State.ExitCode}}',
      'compose down -v',
    ]);
