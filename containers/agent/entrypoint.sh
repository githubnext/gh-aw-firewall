...
# - CAP_NET_ADMIN is NOT present (iptables setup handled by separate init container)
...
if [ "${AWF_CHROOT_ENABLED}" = "true" ]; then
  CAPS_TO_DROP="cap_sys_chroot"
  echo "[entrypoint] Chroot mode enabled - dropping CAP_SYS_CHROOT"
else
  CAPS_TO_DROP=""
  echo "[entrypoint] No capabilities to drop (NET_ADMIN never granted - handled by init container)"
fi
...
AWFEOF
  ...
  # Simply switch to unprivileged user and execute command
