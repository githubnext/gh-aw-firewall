...
# Output:
# [INFO] Runner logs preserved at: /tmp/runner-logs-<timestamp>
# [INFO] Squid logs preserved at: /tmp/squid-logs-<timestamp>
```

**Copilot Logs:**
- Contains Copilot CLI debug output and session information
- Location: `/tmp/runner-logs-<timestamp>/`
- View with: `cat /tmp/runner-logs-<timestamp>/*.log`
...
- Volume mounts map these to `${workDir}/runner-logs/` and `${workDir}/squid-logs/`
